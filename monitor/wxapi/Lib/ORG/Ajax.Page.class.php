...
class Page {
    ...
    public $ajaxLinkPage = false; // 是否是ajax分页
     ...
     * @return string
     * 2016.03.02 - by jackey => 这里使用的是2.1内核，而分页是3.3的内核，所以这里需要去掉 urlencode()...
     */
    ...
    //
    // 设置ajax定制的链接...
    public function setAjaxPage($ajaxPage) {
      $this->ajaxLinkPage = $ajaxPage;
    }
    ...
    public function show() {
        ...
        if( !$this->ajaxLinkPage ) {
          $this->url = U(ACTION_NAME, $this->parameter);
        } else {
          $this->url = str_replace('%PAGE%', $this->parameter[$this->p], $this->ajaxLinkPage);
        }
        /* 计算分页信息 */
