...
//
// 获取json对象的字符串格式...
string CUtilTool::getJsonString(Json::Value & inValue)
{
	string strReturn;
	TCHAR szBuffer[32] = {0};
	if( inValue.isInt() ) {
...
LPCTSTR	CUtilTool::GetServerVersion()
{
	static TCHAR szBuffer[MAX_PATH] = {0};
	if( strlen(szBuffer) > 0 )
		return szBuffer;
	::sprintf(szBuffer, "%s", _T(SZ_VERSION_NAME));
	ASSERT( strlen(szBuffer) < MAX_PATH );
	return szBuffer;
}

LPCTSTR	CUtilTool::GetServerName() {
	return _T(SERVER_NAME);
}
