...
bool LibMP4::CreateAudioTrack(const char* lpUTF8Name, int nAudioSampleRate, string & strAES)
{
	...
	// 2017.07.06 - by jackey => 音频也采用计算时间差的方式...
	m_audioID = MP4AddAudioTrack(m_hFileHandle, nAudioSampleRate, MP4_INVALID_DURATION, MP4_MPEG4_AUDIO_TYPE);
...
bool LibMP4::WriteSample(bool bIsVideo, BYTE * lpFrame, int nSize, uint32_t inTimeStamp, bool bIsKeyFrame)
{
		...
		if( !bIsVideo ) {
			...
			if( m_deqAudio.size() >= 500 ) {
				...
				for(itor = m_deqAudio.begin(); itor != m_deqAudio.end(); ++itor) {
					...
					// 2017.07.06 - by jackey => 单音频模式直接用1024数字...
					MP4WriteSample(m_hFileHandle, m_audioID, (BYTE*)myFrame.m_strData.c_str(), myFrame.m_strData.size(), 1024, 0, myFrame.m_bKeyFrame);
	...
	if( bIsVideo ) { 
		...
		string & strData = m_VLastFrame.m_strData;
		bWriteFlag = MP4WriteSample(m_hFileHandle, theTrackID, (BYTE*)strData.c_str(), strData.size(), uDuration, 0, m_VLastFrame.m_bKeyFrame);
		// 计算写盘量和总时间...
		...
		// 判断是否需要保存第一帧...
		if( m_ALastFrame.m_strData.size() <= 0 ) {
			m_ALastFrame.m_bKeyFrame = bIsKeyFrame;
			m_ALastFrame.m_strData.assign((char*)lpFrame, nSize);
			m_ALastFrame.m_nTimeStamp = inTimeStamp;
			return true;
		}
		// 准备需要的时间戳数据内容...
		// 准备需要的数据内容...
		int			uFrameMS = inTimeStamp - m_ALastFrame.m_nTimeStamp;
		MP4Duration uDuration = uFrameMS * m_nAudioSampleRate / 1000;
		if( uFrameMS <= 0 ) uDuration = 1;
		//TRACE("Audio-Duration = %I64d, size = %lu, keyFrame = %d\n", uDuration, m_ALastFrame.m_strData.size(), m_ALastFrame.m_bKeyFrame);

		// 保存上一帧的音频数据...
		string & strData = m_ALastFrame.m_strData;
		bWriteFlag = MP4WriteSample(m_hFileHandle, theTrackID, (BYTE*)strData.c_str(), strData.size(), uDuration, 0, m_ALastFrame.m_bKeyFrame);
