...
static __inline void circlebuf_init(struct circlebuf *cb)
{
	memset(cb, 0, sizeof(struct circlebuf));
}

static __inline void circlebuf_free(struct circlebuf *cb)
{
	bfree(cb->data);
	memset(cb, 0, sizeof(struct circlebuf));
}

static __inline void circlebuf_reorder_data(struct circlebuf *cb,
		size_t new_capacity)
{
	size_t difference;
	uint8_t *data;
...
static __inline void circlebuf_ensure_capacity(struct circlebuf *cb)
{
	size_t new_capacity;
...
static __inline void circlebuf_reserve(struct circlebuf *cb, size_t capacity)
{
...
static __inline void circlebuf_upsize(struct circlebuf *cb, size_t size)
{
...
static __inline void circlebuf_place(struct circlebuf *cb, size_t position,
...
static __inline void circlebuf_push_front(struct circlebuf *cb, const void *data,
		size_t size)
{
	cb->size += size;
	circlebuf_ensure_capacity(cb);
...
static __inline void circlebuf_push_back_zero(struct circlebuf *cb, size_t size)
{
	size_t new_end_pos = cb->end_pos + size;
...
static __inline void circlebuf_push_front_zero(struct circlebuf *cb, size_t size)
{
	cb->size += size;
	circlebuf_ensure_capacity(cb);
...
static __inline void circlebuf_peek_front(struct circlebuf *cb, void *data,
		size_t size)
{
	assert(size <= cb->size);
...
static __inline void circlebuf_peek_back(struct circlebuf *cb, void *data,
		size_t size)
{
	assert(size <= cb->size);
...
static __inline void circlebuf_pop_front(struct circlebuf *cb, void *data,
		size_t size)
{
...
static __inline void circlebuf_pop_back(struct circlebuf *cb, void *data,
		size_t size)
{
	circlebuf_peek_front(cb, data, size);
...
static __inline void *circlebuf_data(struct circlebuf *cb, size_t idx)
{
