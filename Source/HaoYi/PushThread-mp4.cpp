...
MP4TrackId		g_nAudioID = 0;
MP4TrackId      g_nVideoID = 0;
MP4FileHandle	g_hFileHandle = NULL;
...
CMP4Thread::~CMP4Thread()
{
	if( g_hFileHandle != MP4_INVALID_FILE_HANDLE ) {
		MP4Close(g_hFileHandle);
		g_hFileHandle = MP4_INVALID_FILE_HANDLE;
	}
...
bool CMP4Thread::ReadOneFrameFromMP4(MP4TrackId tid, uint32_t sid, bool bIsVideo, uint32_t & outSendTime)
{
	...
	// 2017.07.06 - by jackey => test...
	if( bIsVideo )
		MP4WriteSample(g_hFileHandle, g_nVideoID, pSampleData, nSampleSize, nDuration, 0, bIsKeyFrame);
...
{
	if( g_hFileHandle == MP4_INVALID_FILE_HANDLE ) {
		g_hFileHandle = MP4Create("F:\\GMSave\\1.mp4");
	}
...
bool CMP4Thread::doMP4ParseAV(MP4FileHandle inFile)
{
    ...
    for( uint32_t i = 0; i < trackCount; ++i ) {
		...
		if( MP4_IS_VIDEO_TRACK_TYPE( type ) ) {
			...
			g_nVideoID = MP4CloneTrack(inFile, id, g_hFileHandle);

			// 2017.07.06 - by jackey => test...
			/*BYTE * lpXSPS = (BYTE*)m_strSPS.c_str();
			uint32_t timescale = MP4GetTrackTimeScale( inFile, id );
			MP4SetTimeScale(g_hFileHandle, timescale);
			g_nVideoID = MP4AddH264VideoTrack(g_hFileHandle, 
											timescale, 
											...
											lpXSPS[3],
											3);
			uint8_t outProfile, outLevel, nVProLevel;
			uint32_t sampleLenFieldSizeMinusOne;
			MP4GetTrackH264LengthSize(inFile, id, &sampleLenFieldSizeMinusOne);
			MP4GetTrackH264ProfileLevel(inFile, id, &outProfile, &outLevel);
			nVProLevel = MP4GetVideoProfileLevel(inFile, id);
			MP4SetVideoProfileLevel(g_hFileHandle, nVProLevel);//0x7F);
			MP4AddH264SequenceParameterSet(g_hFileHandle, g_nVideoID, (BYTE*)m_strSPS.c_str(), m_strSPS.size());
			MP4AddH264PictureParameterSet(g_hFileHandle, g_nVideoID, (BYTE*)m_strPPS.c_str(), m_strPPS.size());*/
		}
			...
			// 2017.07.06 - by jackey => test...
			//g_nAudioID = MP4AddAudioTrack(g_hFileHandle, timeScale, MP4_INVALID_DURATION, MP4_MPEG4_AUDIO_TYPE);
			//uint8_t nALevel = MP4GetAudioProfileLevel(g_hFileHandle);
			//MP4SetAudioProfileLevel(g_hFileHandle, nALevel);// 0x02);//0xFF);
			//MP4SetTrackESConfiguration(g_hFileHandle, g_nAudioID, (BYTE*)m_strAES.c_str(), m_strAES.size());
		}
	}
	return true;
}
