...
BOOL CSortListCtrl::SetHeadings( UINT uiStringID, int nFormat/* = LVCFMT_CENTER */)
{
	CString strHeadings;
	VERIFY( strHeadings.LoadString( uiStringID ) );
	return SetHeadings( strHeadings, nFormat );
}
								...
								int nFormat/* = LVCFMT_CENTER */)
{
	...
	for( ;; )
	{
		...
		if( InsertColumn( m_iNumColumns++, strHeading, nFormat, iWidth ) == -1 )
			return FALSE;
	}
...
int CSortListCtrl::AddItem( int nImage, LPCTSTR pszText, ... )
{
	const int iIndex = InsertItem( GetItemCount(), pszText , nImage);
	...
	for( int iColumn = 1; iColumn < m_iNumColumns; iColumn++ )
	{
		...
		VERIFY( CListCtrl::SetItem( iIndex, iColumn, LVIF_TEXT|LVIF_IMAGE, pszText, 0, 0, 0, 0 ) );
...
void CSortListCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
